<section class="py-16 bg-[#F6F5F2]" data-aos="fade-up" data-aos-duration="1000">
    <div class="container mx-auto px-4">
        <div class="flex flex-col items-center mb-12">
            <span
                class="px-4 py-1 rounded-full bg-gray-100 text-gray-700 text-md outline outline-1 outline-gray-500 font-semibold mb-4">Industries
                We
                Serve
            </span>
            <h2 class="text-2xl md:text-3xl font-bold text-center mb-4">Discover how Becker's vacuum
                solutions
                empower
                <br>various industries worldwide.
            </h2>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            <!-- Card 1 -->
            <div class="bg-[#F6F5F2] hover:bg-[#1e3a8a] rounded-2xl flex flex-col group overflow-hidden rounded-t-4xl transition-all duration-300 transform hover:scale-105"
                data-aos="fade-up" data-aos-delay="0">
                <img src="{{ asset('images/homepage/dental.jpg') }}" alt="Service 1"
                    class="w-full h-72 object-cover rounded-4xl" loading="lazy">
                <div
                    class="w-full flex justify-between items-center mt-auto px-4 py-8 transition-colors duration-200 group-hover:bg-[#1e3a8a]">
                    <span
                        class="text-xl font-semibold text-[#23403A] group-hover:text-white transition-colors duration-200">Medical</span>
                    <span
                        class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 group-hover:bg-white transition-colors duration-200">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24">
                            <path d="M7 7h8.586L5.293 17.293l1.414 1.414L17 8.414V17h2V5H7v2z" />
                        </svg>
                    </span>
                </div>
            </div>
            <!-- Card 2 -->
            <div class="bg-[#F6F5F2] hover:bg-[#1e3a8a] rounded-2xl flex flex-col group overflow-hidden rounded-t-4xl transition-all duration-300 transform hover:scale-105"
                data-aos="fade-up" data-aos-delay="100">
                <img src="{{ asset('images/homepage/wood.jpg') }}" alt="Service 2"
                    class="w-full h-72 object-cover rounded-4xl" loading="lazy">
                <div
                    class="w-full flex justify-between items-center mt-auto px-4 py-8 transition-colors duration-300 group-hover:bg-[#1e3a8a] ">
                    <span
                        class="text-xl font-semibold text-[#23403A] group-hover:text-white transition-colors duration-200">Wood
                        Working</span>
                    <span
                        class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 group-hover:bg-white transition-colors duration-200">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24">
                            <path d="M7 7h8.586L5.293 17.293l1.414 1.414L17 8.414V17h2V5H7v2z" />
                        </svg>
                    </span>
                </div>
            </div>
            <!-- Card 3 -->
            <div class="bg-[#F6F5F2] hover:bg-[#1e3a8a] rounded-2xl  flex flex-col group overflow-hidden rounded-t-4xl transition-all duration-300 transform hover:scale-105"
                data-aos="fade-up" data-aos-delay="200">
                <img src="{{ asset('images/homepage/medicine.jpg') }}" alt="Service 3"
                    class="w-full h-72 object-cover rounded-4xl" loading="lazy">
                <div
                    class="w-full flex justify-between items-center mt-auto px-4 py-8 transition-colors duration-200 group-hover:bg-[#1e3a8a]">
                    <span
                        class="text-xl font-semibold text-[#23403A] group-hover:text-white transition-colors duration-200">Pharmaceutical</span>
                    <span
                        class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 group-hover:bg-white transition-colors duration-200">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24">
                            <path d="M7 7h8.586L5.293 17.293l1.414 1.414L17 8.414V17h2V5H7v2z" />
                        </svg>
                    </span>
                </div>
            </div>
            <!-- Card 4 -->
            <div class="bg-[#F6F5F2] hover:bg-[#1e3a8a] rounded-2xl  flex flex-col group overflow-hidden rounded-t-4xl transition-all duration-300 transform hover:scale-105"
                data-aos="fade-up" data-aos-delay="300">
                <img src="{{ asset('images/homepage/brewer.jpg') }}" alt="Service 4"
                    class="w-full h-72 object-cover rounded-4xl" loading="lazy">
                <div
                    class="w-full flex justify-between items-center mt-auto px-4 py-8 transition-colors duration-200 group-hover:bg-[#1e3a8a]">
                    <span
                        class="text-lg font-semibold text-[#23403A] group-hover:text-white transition-colors duration-200">Distillation
                    </span>
                    <span
                        class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 group-hover:bg-white transition-colors duration-200">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24">
                            <path d="M7 7h8.586L5.293 17.293l1.414 1.414L17 8.414V17h2V5H7v2z" />
                        </svg>
                    </span>
                </div>
            </div>
        </div>
        <div class="flex justify-center">
            <div class="mt-8">
                <a href="{{ route('services') }}"
                    class="inline-block bg-[#1e3a8a] text-white font-semibold py-3 px-8 rounded-full hover:bg-[#2563eb] transition">
                    Explore Our Services
                </a>
            </div>
        </div>
    </div>
</section>